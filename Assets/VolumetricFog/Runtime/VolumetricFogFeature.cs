...
namespace Andicraft.VolumetricFog
{
    public class VolumetricFogFeature : ScriptableRendererFeature
    {
        ...
        [Range(3, 128)] public int raymarchSteps = 16;
        [Min(50)] public float maxDistance = 250;
        [Min(25)] public float lowestMipDistance = 100;
        [Min(0)] public float lowestMip = 1;
        [Min(0.001f)] public float blurDepthFalloff = 1.0f;

        [Space(16)] [Tooltip("VERY expensive. You should probably not use this.")]
        public bool selfShadowing = false;

        [Range(1, 8)] public int directionalShadowSteps = 2;
        [Range(1, 8)] public int additionalLightShadowSteps = 2;

        public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
        {
            if (renderingData.cameraData.isPreviewCamera == false ||
                renderingData.cameraData.cameraType == CameraType.Reflection)
            {
                ...
                if (renderMaterial == null || !renderMaterial)
                {
        ...
        class VolumetricFogRenderPass : ScriptableRenderPass
        {
            ...
            public void Setup(RenderingData renderingData, Material mat, ShaderParameters p)
            {
                ...
                RenderingUtils.ReAllocateIfNeeded(ref copiedColor, colorCopyDescriptor,
                ...
                RenderingUtils.ReAllocateIfNeeded(ref halfDepthBuffer, halfDepthDescriptor,
                    name: "_VolumetricFogBuffer3");
            }
 ...
 renderingData.cameraData.xr.enabled && renderingData.cameraData.xr.singlePassEnabled ? 2 : 1;
                ...
                renderMaterial.SetVector(s_ProjectionParams2ID,
                ...
                else vfog = FindObjectOfType<VolumetricFog>();

                if (vfog && vfog.volumes.Count > 0)
                {
                    if (m_FogVolumesBuffer != null) m_FogVolumesBuffer.Dispose();
                    m_FogVolumesBuffer = new ComputeBuffer(vfog.volumes.Count, FogDensityVolume.BufferStride,
                        ComputeBufferType.Structured, ComputeBufferMode.Immutable);
                    FogDensityVolume.FogVolumeBufferStruct[] structs =
                        new FogDensityVolume.FogVolumeBufferStruct[vfog.volumes.Count];
            ...
            public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
            {
                ...
                Blitter.BlitCameraTexture2D(cmd, source, copiedColor);

                // Create half-size depth target
                Blitter.BlitCameraTexture(cmd, cameraData.renderer.cameraDepthTargetHandle, halfDepthBuffer,
                    renderMaterial,
                    copyDepthPass);
                ...
                Blitter.BlitCameraTexture(cmd, cameraData.renderer.cameraColorTargetHandle, fogBuffer, renderMaterial,
                    renderFogPass);

                // Blur Fog
                renderMaterial.SetTexture(VFogBuffer, fogBuffer);
                ...
                // Apply Fog
                renderMaterial.SetTexture(BlitTexture, copiedColor);
                Blitter.BlitCameraTexture(cmd, copiedColor, cameraData.renderer.cameraColorTargetHandle, renderMaterial,
                    applyFogPass);
        ...
        private static readonly int s_blueNoiseOffset = Shader.PropertyToID("_VFogBlueNoiseOffset");
        ...
        private static readonly int VFogVolumes = Shader.PropertyToID("_VFogVolumes");
        private static readonly int HalfDepthBuffer = Shader.PropertyToID("_HalfDepthBuffer");
        private static readonly int VFogBuffer = Shader.PropertyToID("_VFogBuffer");
        private static readonly int BlurBuffer = Shader.PropertyToID("_BlurBuffer");
        private static readonly int BlitTexture = Shader.PropertyToID("_BlitTexture");
        private static readonly int VFogHeightNoiseRange = Shader.PropertyToID("_VFogHeightNoiseRange");
        private static readonly int VFogHeightNoiseScale = Shader.PropertyToID("_VFogHeightNoiseScale");
    }
}