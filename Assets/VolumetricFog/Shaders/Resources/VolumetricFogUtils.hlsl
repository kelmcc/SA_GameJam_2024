...
float2 _VFogHeightNoiseRange;
float _VFogHeightNoiseScale;
...
float2 _VFogBlueNoiseOffset;

float BlueNoise(float2 uv)
{
    const float2 coords = ((uv+_VFogBlueNoiseOffset) * float2(_ScreenParams.xy)) / 256.0;
...
float getCornetteShanks(float costheta)
{
    ...
    float term2 = (1 + cos2) / (pow((1 + g2 - 2 * _VFogAnisotropy * cos2), 3 / 2));
    return term1 * term2;
}
...
float4 Blur(float2 uv, float2 delta)
{
    ...
    for (int i = 1; i < 4; i++)
    {
        const float2 thisUv = uv + delta.xy * _BlitTexture_TexelSize.xy * offset[i];

        const float depth = GetHalfDepth(thisUv);
        float w = abs(centralDepth - depth);
        w = exp(-w * w) * _BlurDepthFalloff;
        const float contribution = w * weight[i];
        result += SAMPLE_TEXTURE2D_X(_BlitTexture, sampler_LinearClamp,
                                     thisUv) * contribution;
