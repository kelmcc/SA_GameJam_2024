    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings inp) : SV_Target
            {
                ...
                // half3 camPos = _WorldSpaceCameraPos+ ReconstructViewPos(inp.texcoord, 0);

                const half3 viewDir = normalize(viewPos);
                ...
                const float3 startPos = _WorldSpaceCameraPos;
                float3 rayPos = startPos;
                const float3 rayDir = viewDir;
                const float rayLength = min(pixelDepth, _MaxDistance);

                const float stepSize = rayLength / _RaymarchSteps;
                const float3 step = rayDir * stepSize;
                ...
                [loop]
                for (uint i = 0; i < (uint) _RaymarchSteps; i++)
                {
                    [branch]
                    if (totalDistance > pixelDepth) break; // This _shouldn't_ happen but if it does...
                    
                    const float density = GetDensity(rayPos, fogColor);
                    const float extinction = max(0.000001, _VFogExtinction + density);
        ...
        Pass
        {
            ...
            {
                UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
                const float noise1 = remap(0,1,-1,1,BlueNoise(input.texcoord));
                const float noise2 = remap(0,1,-1,1,BlueNoise(input.texcoord + float2(0.3, 0.2)));
                const float2 noiseOffset = 4*float2(noise1, noise2)*_BlitTexture_TexelSize.xy;
                const float2 uv = noiseOffset+input.texcoord;
                float4 fog = GetNearestColor(uv);
