...
{
    float3 c = SAMPLE_TEXTURE3D_LOD(_VFogCurlTexture, sampler_LinearRepeat, (pos + _Time.y * _VFogCurlSpeed) / _VFogCurlSize, 0).rgb;
    c = c * 0.5 - float3(1,1,1);
...
{
    pos += Curl(pos);
    const float4 noiseTex = SAMPLE_TEXTURE3D_LOD(_VFogNoiseTexture, sampler_LinearRepeat, (pos + _Time.y * _VFogNoiseSpeed) / _VFogNoiseSize, 0);
...
float GetDensity(float3 pos, out float3 fogColor)
{
    ...
    float3 col = _VFogColor.rgb;
        ...
        [branch]
        if (1 - hlerp > 0)
            noise = remap(0,1, _VFogHeightNoiseRange.x, _VFogHeightNoiseRange.y, GetNoise(_VFogHeightNoiseScale.xxx*pos));
