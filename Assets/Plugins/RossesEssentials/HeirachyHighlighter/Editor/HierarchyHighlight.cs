    ...
    public class HierarchyHighlightManager
    {
        ...
        private static void HierarchyHighlight_OnGUI(int inSelectionID, Rect inSelectionRect)
        {
            ...
            if (Label != null && Event.current.type == EventType.Repaint)
            {
                ...
                Rect IconOffset = new Rect(inSelectionRect.position + new Vector2(0f, 0f), inSelectionRect.size);
                Rect TextOffset = new Rect(inSelectionRect.position + new Vector2(18f, 0f), inSelectionRect.size);
                ...
                var icon = Label.Icon == null ? AssetPreview.GetMiniThumbnail(GO_Label) : Label.Icon;
                GUIContent iconGUIContent = new GUIContent(icon);
                EditorGUI.LabelField(IconOffset, iconGUIContent);
                EditorGUI.LabelField(TextOffset, GO_Label.name, new GUIStyle()
                {
